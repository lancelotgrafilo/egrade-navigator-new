...
export function FacultyDashboard() {
  ...
  const handleConfirmLogout = async () => {
    try {
      ...
      await axios.patch(`https://egrade-backend.onrender.com/api/update-faculty-staff-status/${userId}`, { isActive: false });
  ...
  useEffect(() => {
    const fetchUserProfileImage = async () => {
      try {
        const response = await axios.get(`https://egrade-backend.onrender.com/api/get_faculty_details/${userId}`);
